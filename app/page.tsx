...
type View = "dashboard" | "inventory" | "expiring" | "shopping" | "recipes" | "settings" | "menu";
...
export default function Home() {
  ...
  const addRecipe = (recipe: Omit<Recipe, "id">) => {
    ...
    void apiAction("addRecipe", { recipe });
  ...
  const renderView = () => {
    ...
    if (view === "recipes") return <RecipesView mode={recipeMode} setMode={setRecipeMode} recipes={recipeList} inventory={inventory} onOpen={setActiveRecipe} onAddMissing={addMissing} onAdd={() => setShowRecipeForm(true)} />;
    if (view === "settings") return <SettingsView inventory={inventory} shopping={shopping} />;
    if (view === "menu") return <MobileMenuView databaseState={databaseState} onNavigate={setView} />;
...
function RecipesView({ mode, setMode, recipes: recipeList, inventory, onOpen, onAddMissing, onAdd }: { mode: RecipeMode; setMode: (mode: RecipeMode) => void; recipes: Recipe[]; inventory: InventoryItem[]; onOpen: (recipe: Recipe) => void; onAddMissing: (recipe: Recipe) => void; onAdd: () => void }) { const sorted = [...recipeList].sort((a, b) => { const aMissing = missingIngredients(a, inventory).length; const bMissing = missingIngredients(b, inventory).length; if (mode === "minimal-shopping") return aMissing - bMissing; if (mode === "use-soon") return matchingIngredients(b, inventory).length - matchingIngredients(a, inventory).length; return matchingIngredients(b, inventory).length - matchingIngredients(a, inventory).length; }); return <div className="list-view recipe-view"><div className="page-intro"><div><div className="eyebrow">Good food, less guesswork</div><h1>Recipes</h1><p>Ideas built around what&apos;s already waiting in your kitchen.</p></div><div className="recipe-heading-actions"><span className="recipe-badge">✦ English + magyar</span><button className="primary-button" onClick={onAdd}>＋ Add recipe</button></div></div><div className="mode-tabs">{([["use-soon", "Cook with expiring items"], ["use-what-i-have", "Use what I have"], ["minimal-shopping", "Minimal shopping"]] as [RecipeMode, string][]).map(([id, label]) => <button key={id} className={mode === id ? "active" : ""} onClick={() => setMode(id)}>{label}</button>)}</div><div className="recipe-grid">{sorted.length ? sorted.map((recipe) => <RecipeCard key={recipe.id} recipe={recipe} inventory={inventory} onOpen={() => onOpen(recipe)} onAddMissing={() => onAddMissing(recipe)} />) : <EmptyState title="No recipes yet" body="Add your own recipes in English and Hungarian." action={<button className="secondary-button" onClick={onAdd}>＋ Add your first recipe</button>} />}</div></div>; }
function RecipeCard({ recipe, inventory, onOpen, onAddMissing }: { recipe: Recipe; inventory: InventoryItem[]; onOpen: () => void; onAddMissing: () => void }) { const matching = matchingIngredients(recipe, inventory); const missing = missingIngredients(recipe, inventory); return <article className="recipe-card"><div className="recipe-card-top"><div className="recipe-symbol">✦</div><div><div className="recipe-tags">{recipe.tags.map((tag) => <span key={tag}>{tag}</span>)}</div><h2>{recipe.name}</h2>{recipe.nameHu && <span className="recipe-localized-name">{recipe.nameHu}</span>}<p>{recipe.description}</p>{recipe.descriptionHu && <p className="recipe-localized-copy">{recipe.descriptionHu}</p>}</div></div><div className="ingredient-summary"><div><span className="ingredient-label">You have</span><strong>{matching.length} / {recipe.ingredients.length} ingredients</strong></div><div className="ingredient-bar"><span style={{ width: `${recipe.ingredients.length ? (matching.length / recipe.ingredients.length) * 100 : 0}%` }} /></div></div><div className="recipe-card-meta"><span>◷ {recipe.time}</span><span>⌁ {recipe.difficulty}</span><span>{missing.length ? `${missing.length} to buy` : "Ready to cook"}</span></div><div className="recipe-card-actions"><button className="secondary-button" onClick={onOpen}>View recipe</button>{missing.length > 0 && <button className="text-button" onClick={onAddMissing}>＋ Add missing</button>}</div></article>; }

function MobileMenuView({ databaseState, onNavigate }: { databaseState: "checking" | "connected" | "offline"; onNavigate: (view: View) => void }) { return <div className="list-view mobile-menu-view"><div className="page-intro"><div><div className="eyebrow">Workspace</div><h1>Menu</h1><p>Recipes, settings and the rest of your HomeStock workspace.</p></div></div><section className="panel menu-panel"><button className="menu-link" onClick={() => onNavigate("recipes")}><span className="menu-link-icon">✦</span><span><strong>Recipes</strong><small>Add and browse English + magyar recipes</small></span><b>→</b></button><button className="menu-link" onClick={() => onNavigate("settings")}><span className="menu-link-icon">⚙</span><span><strong>Settings</strong><small>Workspace preferences and database snapshot</small></span><b>→</b></button><div className="menu-database-status"><span className={`storage-orb ${databaseState}`} /><div><strong>{databaseState === "connected" ? "Turso workspace" : databaseState === "checking" ? "Checking database" : "Database not connected"}</strong><small>{databaseState === "connected" ? "Changes sync across devices" : "Add the Vercel database variables to enable sync"}</small></div></div></section></div>; }

function RecipeFormModal({ onClose, onSubmit }: { onClose: () => void; onSubmit: (recipe: Omit<Recipe, "id">) => void }) {
  ...
  const [ingredientsHu, setIngredientsHu] = useState("");
  const [stepsEn, setStepsEn] = useState("");
  const [stepsHu, setStepsHu] = useState("");
  const [tagsEn, setTagsEn] = useState("");
  const [tagsHu, setTagsHu] = useState("");
...
function RecipeModal({ recipe, inventory, onClose, onAddMissing, onFinish }: { recipe: Recipe; inventory: InventoryItem[]; onClose: () => void; onAddMissing: () => void; onFinish: () => void }) { const matching = matchingIngredients(recipe, inventory); const missing = missingIngredients(recipe, inventory); return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><div className="modal-card recipe-modal"><div className="modal-header"><div><div className="eyebrow">Cook with what you have</div><h2>{recipe.name}</h2>{recipe.nameHu && <span className="recipe-localized-name">{recipe.nameHu}</span>}</div><IconButton label="Close" onClick={onClose}>×</IconButton></div><p className="recipe-modal-description">{recipe.description}</p>{recipe.descriptionHu && <p className="recipe-modal-description recipe-localized-copy">{recipe.descriptionHu}</p>}<div className="recipe-modal-meta"><span>◷ {recipe.time}</span><span>⌁ {recipe.difficulty}</span><span>✦ {matching.length} at home</span></div><div className="modal-columns"><div><h3>Ingredients</h3><ul className="ingredient-list">{recipe.ingredients.map((ingredient, index) => <li key={ingredient} className={matching.includes(ingredient) ? "have" : "missing"}><span className="ingredient-check">{matching.includes(ingredient) ? "✓" : "＋"}</span><div><strong>{ingredient}</strong>{recipe.ingredientsHu?.[index] && <small>{recipe.ingredientsHu[index]}</small>}</div>{!matching.includes(ingredient) && <em>to buy</em>}</li>)}</ul>{missing.length > 0 && <button className="text-button" onClick={onAddMissing}>＋ Add {missing.length} missing to list</button>}</div><div><h3>How to make it</h3><ol className="steps-list">{recipe.steps.map((step, index) => <li key={step}><strong>{step}</strong>{recipe.stepsHu?.[index] && <span>{recipe.stepsHu[index]}</span>}</li>)}</ol></div></div><div className="modal-actions"><button className="secondary-button" onClick={onClose}>Back</button><button className="primary-button" onClick={onFinish}>Mark ingredients for use</button></div></div></div>; }
