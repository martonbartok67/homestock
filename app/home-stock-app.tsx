...
function ItemRow({ item, onFinish, onDelete, onEditExpiry, compact = false, tapped = false, onTap }: { item: InventoryItem; onFinish: () => void; onDelete: () => void; onEditExpiry: () => void; compact?: boolean; tapped?: boolean; onTap?: () => void }) {
...
export default function Home() {
  ...
  const renderView = () => {
    if (view === "inventory") return <InventoryView inventory={filteredInventory} query={query} setQuery={setQuery} categoryFilter={categoryFilter} setCategoryFilter={setCategoryFilter} onAdd={() => setShowAdd(true)} onFinish={finishItem} onDelete={deleteItem} onEditExpiry={setEditingExpiryItem} />;
    if (view === "expiring") return <ExpiringView items={expiring} onFinish={finishItem} onDelete={deleteItem} onAdd={() => setShowAdd(true)} onEditExpiry={setEditingExpiryItem} />;
...
function RecipesView({ mode, setMode, tagFilter, setTagFilter, recipes: recipeList, inventory, suggestedRecipe, suggestionSource, suggestionStatus, isSuggesting, onSuggest, onOpen, onOpenSuggestion, onSaveSuggestion, onEdit, onDelete, onAddMissing, onAdd }: { mode: RecipeMode; setMode: (mode: RecipeMode) => void; tagFilter: string; setTagFilter: (tag: string) => void; recipes: Recipe[]; inventory: InventoryItem[]; suggestedRecipe: Recipe | null; suggestionSource: RecipeSuggestionSource | null; suggestionStatus: string; isSuggesting: boolean; onSuggest: () => void; onOpen: (recipe: Recipe) => void; onOpenSuggestion: () => void; onSaveSuggestion: () => void; onEdit: (recipe: Recipe) => void; onDelete: (recipe: Recipe) => void; onAddMissing: (recipe: Recipe) => void; onAdd: () => void }) {
  ...
  return <div className="list-view recipe-view">
    ...
    <div className="recipe-controls"><div className="mode-tabs">{([["use-soon", "Cook with expiring items"], ["use-what-i-have", "Use what I have"], ["minimal-shopping", "Minimal shopping"]] as [RecipeMode, string][]).map(([id, label]) => <button key={id} className={mode === id ? "active" : ""} onClick={() => setMode(id)}>{label}</button>)}</div><label className="tag-filter">Tag<select value={activeTag} onChange={(event) => setTagFilter(event.target.value)}>{tagOptions.map((tag) => <option key={tag}>{tag}</option>)}</select></label></div>
