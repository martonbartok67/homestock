...
app/
  ...
  api/
    ...
    recipes/
      ...
      suggest/route.ts   # POST — AI/web recipe suggestion (mode, typeFilter, excludeIds)
...
lib/
  ...
  server/
    ...
    online-recipe-suggestion.ts # 4-tier suggestion engine (web search → Gemini grounding → Gemini AI → MealDB)
...
- **Structural tests** (`rendered-html.test.mjs`): read source files as strings and `assert.match` for key patterns. Use this to guard against regressions where a critical wire-up (e.g. `householdFetch("/api/home-stock")`) gets accidentally removed.
...
2. **Never import from `lib/server/` in client code.** This will either cause a build error (Clerk server SDK) or leak secrets. If a type is needed on both sides, extract it to `lib/homestock.ts`.
